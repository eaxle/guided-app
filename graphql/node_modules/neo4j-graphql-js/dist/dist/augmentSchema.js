...
function addRelationshipMutations(type) {
  ...
  relationshipFields.forEach(function (x) {
    ...
    mutations += '\n    Add' + fromType.name + toType.name + '(' + fromPk.name + ': ' + innerType(fromPk.type).name + '!, ' + toPk.name + ': ' + innerType(toPk.type).name + '!): ' + fromType.name + ' @MutationMeta(relationship: "' + relTypeArg.value.value + '", from: "' + fromType.name + '", to: "' + toType.name + '")\n    ';
