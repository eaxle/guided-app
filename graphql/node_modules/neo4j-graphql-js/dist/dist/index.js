...
function cypherMutation(_ref3, context, resolveInfo) {
  ...
  if (mutationTypeCypherDirective) {
    ...
    var fromType = fromTypeArg.value.value,
        ...
        fromParam = resolveInfo.schema.getMutationType().getFields()[resolveInfo.fieldName].astNode.arguments[0].name.value,
        toParam = resolveInfo.schema.getMutationType().getFields()[resolveInfo.fieldName].astNode.arguments[1].name.value;

    var _query = 'MATCH (' + fromVar + ':' + fromType + ' {' + fromParam + ': $' + fromParam + '})\n       MATCH (' + toVar + ':' + toType + ' {' + toParam + ': $' + toParam + '})\n      CREATE (' + fromVar + ')-[:' + relationshipName + ']->(' + toVar + ')\n      RETURN ' + fromVar + ' {' + (0, _selections.buildCypherSelection)({
      initial: '',
      selections: selections,
      variableName: variableName,
      schemaType: schemaType,
